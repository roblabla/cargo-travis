...
pub fn run_coverage(ws: &Workspace,
    ...
    compilation
        .tests
        .sort_by(|a, b| (a.0.package_id(), &a.1).cmp(&(b.0.package_id(), &b.1)));
    ...
    let v: Vec<std::ffi::OsString> = test_args
        .iter()
        .cloned()
        .map::<std::ffi::OsString, _>(|val| val.into())
        .collect();
    ...
    for &(ref pkg, _, _, ref exe) in &compilation.tests {
        ...
        let target = ws.target_dir()
            .join("kcov-".to_string() + to_display.file_name().unwrap().to_str().unwrap())
            .into_path_unlocked();
        let default_include_path = format!("--include-path={}", ws.root().display());
        let mut args: Vec<&std::ffi::OsStr> = vec!["--verify".as_ref(),
                                                   default_include_path.as_ref(),
                                                   target.as_ref()];
    ...
    let mut mergeargs: Vec<OsString> = vec!["--merge".to_string().into(),
                                            options.merge_dir.as_os_str().to_os_string()];
    ...
    try!(config
             .shell()
             .verbose(|shell| shell.status("Merging coverage", cmd.to_string())));
    try!(cmd.exec());
    if errors.is_empty() {
        Ok(None)
    } else {
